...
class GraphDataset(Dataset):
	...
	def __getitem__(self, index):
		...
		tmp_label[0] = label
		return (torch.from_numpy(self.lap_mats1[index]).float(),
		        torch.from_numpy(self.lap_mats2[index]).float(),
		        torch.from_numpy(self.adjs1[index]),
		        torch.from_numpy(self.adjs2[index]),
